...
class DashboardWindow:
    ...
    def setup_ui(self):
        ...
        button_images = [
            ("button_6.png", lambda: print("button_6 clicked")),
            ("button_7.png", lambda: print("button_7 clicked")),
            ("button_8.png", lambda: print("button_8 clicked"))
        ]
...
class AddPaymentWindow:
    ...
    def create_payment_form(self):
        ...
        self.canvas.create_text(50.0, 310.0, anchor="nw", text="Payment Method:", fill="#000000", font=("Arial", 12))
        self.payment_method = ttk.Combobox(self.window, values=["BDO", "GCash", "PayMaya"], state="readonly")
        self.payment_method.place(x=200, y=310, width=200)
        self.payment_method.bind("<<ComboboxSelected>>", self.update_account_entry)

        # Account Entry Frame
        self.account_frame = Label(self.window, text="", font=("Arial", 12), bg="#FFFFFF")
        self.account_frame.place(x=200, y=340)
        self.account_entry = Entry(self.window, font=("Arial", 12))
        self.account_entry.place(x=200, y=365, width=200)

        # Fees Section
        self.canvas.create_text(50.0, 410.0, anchor="nw", text="Fees:", fill="#000000", font=("Arial", 12))
        self.fees_listbox = ttk.Combobox(self.window, state="readonly", font=("Arial", 12))
        self.fees_listbox.place(x=200, y=410, width=200)
        self.fees_listbox.bind("<<ComboboxSelected>>", self.update_fee_balance)

        # Selected Fee
        self.canvas.create_text(50.0, 450.0, anchor="nw", text="Selected Fee:", fill="#000000", font=("Arial", 12))
        self.selected_fee_label = Label(self.window, text="", font=("Arial", 12), bg="#FFFFFF")
        self.selected_fee_label.place(x=200, y=450)

        # Fee Amount
        self.canvas.create_text(50.0, 490.0, anchor="nw", text="Fee Amount:", fill="#000000", font=("Arial", 12))
        self.fee_amount_label = Label(self.window, text="₱0.00", font=("Arial", 12), bg="#FFFFFF")
        self.fee_amount_label.place(x=200, y=490)

        # Amount Paid
        self.canvas.create_text(50.0, 520.0, anchor="nw", text="Amount Paid:", fill="#000000", font=("Arial", 12))
        self.amount_entry = Entry(self.window, bd=1, bg="#FFFFFF", fg="#000716", highlightthickness=0, font=("Arial", 12))
        self.amount_entry.place(x=200, y=520, width=200, height=25)
        self.amount_entry.bind("<KeyRelease>", self.update_remaining_balance)

        # Remaining Balance
        self.canvas.create_text(50.0, 550.0, anchor="nw", text="Remaining Balance:", fill="#000000", font=("Arial", 12))
        ...
        self.remaining_balance_label.place(x=200, y=550)

        # Date
        self.canvas.create_text(50.0, 580.0, anchor="nw", text="Date:", fill="#000000", font=("Arial", 12))
        current_date = datetime.datetime.now().strftime("%Y-%m-%d")
        self.date_label = Label(self.window, text=current_date, font=("Arial", 12), bg="#FFFFFF")
        self.date_label.place(x=200, y=580)
    ...
    def update_account_entry(self, event=None):
        method = self.payment_method.get()
        if method == "BDO":
            self.account_frame.config(text="Enter BDO Account Number:")
            self.account_entry.delete(0, END)
        elif method in ["GCash", "PayMaya"]:
            self.account_frame.config(text="Enter Mobile Number (11 digits):")
            self.account_entry.delete(0, END)
    ...
    def submit_payment(self):
        try:
            ...
            balance = float(self.remaining_balance_var.get().replace("₱", "").replace(",", ""))
            ...
            if not account_number:
                raise ValueError(f"Please enter your {payment_method} account details.")

            if payment_method == "BDO":
                if not re.match(r'^\d{10,12}$', account_number):
                    raise ValueError("BDO account number must be 10–12 digits.")
            elif payment_method in ["GCash", "PayMaya"]:
                if not re.match(r'^09\d{9}$', account_number):
                    raise ValueError(f"{payment_method} number must start with '09' and be 11 digits.")

            # Only deduct from balance if tuition is paid
            ...
            (LRN, StudentFullName, Course, SchoolYear, FeeDescription, Fees, AmountPaid, Balance, Create_at, PaymentMethod, AccountNumber)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """
            self.cursor.execute(insert_query, (
                lrn, name, course, school_year, selected_fee, fee_amount,
                amount_paid, balance, payment_date, payment_method, account_number
            ))
            self.connection.commit()
            self.generate_receipt(lrn, name, course, school_year, selected_fee, fee_amount, amount_paid, balance, payment_date, payment_method, account_number)
    ...
    def generate_receipt(self, lrn, name, course, school_year, fee_desc, fee_amount, amount_paid, balance, date, payment_method, account_number):
    ...
    """
        ...
        save_button = tk.Button(button_frame, text="Print Receipt", width=12,
                               command=lambda: self.save_receipt_to_file(receipt_text))
    ...
    def save_receipt_to_file(self, receipt_widget):
        try:
